...
for trial = 1:20
    ...
    for i = 1:V.frmNum
        ...
        tflip = Screen('Flip', W.on, tflip + W.dur100);
    end
    Screen('Flip', W.on, tflip + W.dur100);
    promptResponse(V, W);
    StimOnSet = Screen('Flip', W.on, tflip);
    % --------------- Collect response
    % Response for a main task & confidence rating 
[dResp, dRT, dCorr] = getResponse(C, K, S, W, StimOnSet);
...
if ~fastMode
    window.dur100	= (round(0.10/blanking)-0.5)*blanking;
    ...
    window.dur100	= (round(0.01/blanking)-0.5)*blanking;
...
variables.blockSizes = [32, 16, 12]; %8 12 16 24 32 48 64
...
variables.gridScrambled{1,e}(end+1) = variables.imgDimentions(3);
end
return
...
elseif currentCond.xDP(1,1) ~= currentCond.xDP(2,1) %between
    ...
    imageStream.frames(inv,:) = [maskItems.Dpix(1,inv), maskItems.Dpix(2,inv),...
        maskItems.Dpix(3,inv), criticalItems.Crpix(1,2), maskItems.Dpix(4,inv), maskItems.Dpix(5,inv), maskItems.Spix(1,2)]; 
end
...
device(1).vendorID= 1452;	% Apple Inc.
        ...
        keyboard.main = KbName({'2@', '3#', '5%', '6^'}); % S-s-d-D
